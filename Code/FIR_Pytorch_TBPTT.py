...
#    MCD_data_AR_dat = MCD_data_AR_dat[10000:, :]
    ...
    tr_X_noisy = tr_X + torch.tensor(np.random.normal(size = tr_X.shape, scale = 0.3)).type(torch.cuda.FloatTensor)
    te_X_noisy = te_X + torch.tensor(np.random.normal(size = te_X.shape, scale = 0.3))
    
    '''=========================================================================='''
    # Defining the models
    '''=========================================================================='''
    class Linear_mdl(nn.Module):
    ...
    class Linear_mdl2(nn.Module):
        def __init__(self, in_feat, out_feat):
            super().__init__()            
            self.linear1 = nn.Linear(in_features = in_feat, out_features = 80)
    ...
    net1 = Linear_mdl(in_feat = ar, out_feat = 1).cuda()        
    optimizer = torch.optim.RMSprop(net1.parameters(), lr=0.001, weight_decay = 0.00001)
    ...
    '''=========================================================================='''
    Prediction_Final =  net1(te_X_noisy.type(torch.cuda.FloatTensor)).cpu().detach()
...
np.save('./Final_results/AR_Lin_R2_noisy_tr_clean_te.npy', DNN_R2, allow_pickle=True, fix_imports=True)       
np.save('./Final_results/AR_Lin_R2_noisy_tr_noisy_te.npy', DNN_R2_noisy, allow_pickle=True, fix_imports=True)       
